# app_interface_v1.py
# Versão enxuta e estável — layout horizontal (45%/55%), sem logo/título,
# com linha de categoria (input+botão) compacta e rodapé full-width com todos botões.
...
CSS_FILE = os.path.join("assets", "sia_theme.css")
...
classes_ui = read_classes(CLASSES_FILE)  # manter atualizado
...
# --------- Opções de detecção ---------
with st.expander("⚙️ Opções de detecção", expanded=False):
...
# --------- LAYOUT: imagem (45%) | comandos (55%) ---------
left_col, right_col = st.columns([9, 11], gap="large")  # proporção sugerida; CSS com !important prevalece
...
with right_col:
    ...
    # Criar nova categoria (input + botão) alinhados e compactos
    st.markdown("**Criar nova categoria**")
    st.markdown('<div class="add-row">', unsafe_allow_html=True)
    c_nc1, c_nc2 = st.columns([4, 1], gap="small")
    with c_nc1:
        new_class = st.text_input(" ", value="", placeholder="ex.: cutia, onça-parda, macuco",
                                  label_visibility="collapsed", key="new_class_input")
    with c_nc2:
        add_click = st.button("➕ Adicionar", key="add_class_btn", use_container_width=True)
    st.markdown('</div>', unsafe_allow_html=True)
        ...
        if not detections:
            ...
            for i, d in enumerate(detections):
                with st.expander(f"Detecção #{i+1} — {d['classe']} ({d.get('conf',0):.2f})", expanded=True):
                    ...
                    ordered = [st.session_state.selected_class] + [c for c in classes_ui if c != st.session_state.selected_class]
                    cls = st.selectbox("Categoria desta detecção", ordered, index=0, key=f"cls_{i}")
                    x1, y1, x2, y2 = d["xyxy"]
...
# ===== Rodapé full-width: todos os botões em uma linha =====
st.markdown('<div class="footer-row">', unsafe_allow_html=True)
f1, f2, f3, f4, f5 = st.columns([1, 1, 1, 1, 1], gap="small")
with f1: back_clicked       = st.button("↩️ Voltar", use_container_width=True)
with f2: skip_frame         = st.button("⏭️ Pular frame", use_container_width=True)
with f3: skip_video         = st.button("⏭️⏭️ Pular vídeo", use_container_width=True)
with f4: save_frame_clicked = st.button("💾 Salvar frame", use_container_width=True)
with f5: save_video_clicked = st.button("💾 Salvar VÍDEO", use_container_width=True)
st.markdown('</div>', unsafe_allow_html=True)
...
def jump_to_next_video():
    idx_prefix = prefixes.index(base_prefix_for_frame(image_paths[st.session_state.idx]))
...
# --------- Salvar ---------
if save_frame_clicked:
    # salva só o frame atual
    if (not edited) and st.session_state.selected_class and allow_force:
        edited = fake_fullbox_detection(curr_path, st.session_state.selected_class)

    if edited:
        save_annotation(curr_path, edited, use_val=False)
        st.success("Frame salvo.")
        st.session_state.idx = min(len(image_paths) - 1, st.session_state.idx + 1)
        st.rerun()
    else:
...
if save_video_clicked:
    # aplica categoria a todos os frames do vídeo atual
    if (not edited) and st.session_state.selected_class and allow_force:
        edited = fake_fullbox_detection(curr_path, st.session_state.selected_class)

    if edited:
        curr_prefix = base_prefix_for_frame(image_paths[st.session_state.idx])
        ...
        prog = st.progress(0)
        mdl = load_model_cached(MODEL_PATH)
        saved = 0
        for i, p in enumerate(batch_paths):
            dets = infer_image(mdl, p, conf_thres, iou_thres)
            if not dets and allow_force and st.session_state.selected_class:
        ...
        st.success(f"Vídeo salvo. Anotações aplicadas em {saved} frame(s).")
        jump_to_next_video()
        st.rerun()
    else:
        st.info("Não houve detecções para salvar neste vídeo (tente reduzir confiança ou habilitar 'salvar sem detecção').")
